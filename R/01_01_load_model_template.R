...
load_model_template <- function(model_template_name) {
  ...
  if (model_template_name == "SIM") {
    ...
    model <- model %>%
      ...
      add_variable("delta", init = 0.1) %>%
      add_variable("gamma", init = 0.15) %>%
      add_variable("kappa", init = 1) %>%
      add_variable("pr", init = 1) %>%
      add_variable("Nd", init = 0.001) %>%
      add_variable("Ns", init = 0.001) %>%
      add_variable("Y", init = 0.001)

    # add_variable('Ls', init = 12) %>% add_variable('Ld', init = 12) %>%
    # add_variable('Mh', init = 12) %>% add_variable('Ms', init = 12) %>%
    # add_variable('Nd', init = 128) equations
    model <- model %>%
      add_equation("Cs = Cd") %>%
      add_equation("Is = Id") %>%
      add_equation("Ns = Nd") %>%
    ...
    model <- model %>%
      ...
      add_variable("Ns", init = 0.001) %>%
      add_variable("Y", init = 0.001) %>%
      add_variable("alpha1w", init = 0.8) %>%
      add_variable("alpha1r", init = 0.15)
    ...
    model <- model %>%
      ...
      add_variable("y", init = 0.001) %>%
      add_variable("p", init = 0.001) %>%
      add_variable("NHUC", init = 0.001) %>%
      add_variable("s_E", init = 0.001)
