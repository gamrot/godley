...
diagnostics <- function(models = c("SIM", "PC", "LP", "REG", "OPEN", "BMW", "BMWK", "DIS", "DISINF", "SIMEX"),
                        parameters,
...
")
  ...
  for (m in models) {
    ...
    if (auto_param) {
      ...
      external_values <- setdiff(model_g$variables$name, eqs_separated$lhs)
      external_values <- model_g$variables %>%
      ...
      parameters <- external_values$lhs
    }
    ...
    for (p in parameters) {
      for (met in methods) {
        ...
        for (e in 1:nrow(external_values)) {
          equations_s_param <- append(
            equations_s_param,
            as.formula(
              paste(
                external_values[e, ]$lhs,
                " ~ ",
                external_values[e, ]$rhs
              )
            )
          )
        }
        ...
        for (e in 1:nrow(eqs_separated)) {
          hidden <- c(as.character(eqs_separated[e, ]$rhs))
          names(hidden) <- as.character(eqs_separated[e, ]$lhs)
