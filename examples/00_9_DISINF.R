model_DISINF <- create_model(name = 'DISINF model') 
...
model_DISINF <- prepare(model_DISINF)

## simulation ----

model_DISINF <- simulate(model_DISINF, scenario = 'baseline', max_iter = 350,
                      periods = 200, hidden_tol = 0.1, tol = 1e-20, method = 'Gauss')


plot_series(model = model_DISINF, scenario = 'baseline', from = 1, to = 200,
            expressions = c('y'))
...
model_DISINF <- simulate(model_DISINF, scenario = 'expansion', max_iter = 350,
                      periods = 100, hidden_tol = .1, tol = 1e-15, method = 'Gauss')

plot_series(model = model_DISINF, scenario = 'expansion', from = 1, to = 100,
            expressions = c("inflation = (p - lag(p)) / lag(p)"))
...
model_DISINF <- simulate(model_DISINF, scenario = 'expansion2', max_iter = 350,
                      periods = 100,  tol = 1e-08, method = 'Gauss')

plot_series(model = model_DISINF, scenario = 'expansion2', from = 1, to = 100,
            expressions = c("ydhs_ss = alpha0 / (1 - alpha1 - alpha2*sigma_T * (UC/p))",
                            "ydhs", "c", "s"))
