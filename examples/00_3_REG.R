...
model_REG <- prepare(model_REG)

## simulation ----

model_REG <- simulate(model_REG, scenario = 'baseline', max_iter = 350,
                     periods = 100, hidden_tol = 0.1, tol = 1e-08, method = 'Gauss')



plot_series(model = model_REG, scenario = 'baseline', from = 1, to = 60,
            expressions = c('deltaV_S = V_S - lag(V_S)',
...
model_REG <- simulate(model_REG, scenario = 'expansion', max_iter = 350,
                      periods = 60, hidden_tol = 0.1, tol = 1e-08, method = 'Gauss')


plot_series(model = model_REG, scenario = 'expansion', from = 1, to = 60,
            expressions = c('deltaV_S = V_S - lag(V_S)',
                            'GB_S = TX_S - (G_S + dplyr::lag(r) * dplyr::lag(Bh_S))',
                            'TB_S = X_S - IM_S'))
...
shock2 <- create_shock() %>% 
  add_shock(equation = 'G_S = 25',
            desc = 'An increase in the government expenditures of the South',
            start = 5,
            end = 60) 
...
model_REG <- simulate(model_REG, scenario = 'expansion2', max_iter = 350,
                      periods = 60, hidden_tol = 0.1, tol = 1e-08, method = 'Gauss')

plot_series(model_REG, scenario = 'expansion2', from = 1, to = 60,
            expressions = c('Y_N', 'Y_S'))
...
shock3 <- create_shock() %>% 
  add_shock(equation = 'alpha1_S = .6',
            desc = '',
            start = 5,
            end = 60) 

model_REG <- model_REG %>% 
  add_scenario(name = 'expansion3', origin = 'baseline',origin_period = 100, 
               shocks = shock3)

model_REG <- simulate(model_REG, scenario = 'expansion3', max_iter = 350,
                      periods = 60, hidden_tol = 0.1, tol = 1e-08, method = 'Gauss')

plot_series(model_REG, scenario = 'expansion3', from = 1, to = 60,
            expressions = c('Y_N', 'Y_S'))
