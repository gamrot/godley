...
Function ```prepare()``` prepares model for simulation. \
Function ```simulate_scenario()``` calculates final matrix for provided scenarios. \
...
Note: Instead of manually adding variables and equations to the model (described below in steps 2. and 3.) user can upload model's attributes from templates available in the package. Available model templates are: SIM, PC, LP, REG, OPEN, BMW, BMWK, DIS, DISINF and SIMEX. 
...
### 5. Preparing model
```prepare()``` function allows to check the correctness of inputs provided by the user and prepare model for simulation by creating initial matrix for the scenario and preparing tibble with equations that will be used in simulation. Function returns error if: \
- there are no variables in the model \
- there are no equations in the model \
- there are special characters in model ( ! , >, < , ~, : , ; #, ^  , , (...) ) \
- there are duplicated variables \

Function returns warning if:
- there is at least one variable used in equations but not defined \
- there is at least one variable not used in created equations \
- there is no description for endogenous variable \
- there are no initial value for exogenous variable (in that case value 0 will be applied by default) \
- there is endogenous variable explained by the model in equations more than once \

To prepare model for simulation please use ```prepare()``` function as in example below:
```{r}
#prepare model for simulation
model_sim <- prepare(model = model_sim) 
```
where: \
model — SFC model (required)
...
max_iter — number of iteration in simulation (required) \
periods — number of simulating periods in simulation (required) \
hidden_tol — maximum error tolerated for hidden equations in simulation (required) \
tol — maximum error tolerated for algorithm until algorithm converge in simulation (required) \
method — 'Newton' or 'Gauss' calculation method in simulation (required) \
...
```{r fig, fig.width = 6, fig.high = 4, fig.asp = 1}
#plot results
...
scenario — name of scenario(s) that will be displayed on the chart (required) \
...
from — first period that will be displayed on the chart (required) \
to — last period that will be displayed on the chart (required) \
expressions — variable names or equations that will be displayed on the chart (required) 
...
To add new shock scenario to SFC model and simulate it please use ```add_scenario()``` and  ```simulate_scenario()``` functions as in example below:
...
name — name of a new scenario (required) \
origin — name of baseline scenario (required) \
origin_period — period from the baseline scenario which will be used as initial period in new scenario (required) \
shock — name of created SFC_shock object (required)

## Model — sensitivity
Sensitivity allows to create in a new SFC model multiple scenarios with changed initial value for selected variable. To create and simulate sensitivity please use ```create_sensitivity()``` and  ```simulate_scenario()``` functions as in example below:
...
scenario — name of the scenario that will be used as a baseline in sensitivity calculation (required) \
variable — name of the variable that will be check in sensitivity calculation (required) \
lower — minimum value of the variable that will be used in sensitivity calculation (required) \
upper — maximum value of the variable that will be used in sensitivity calculation (required) \
step — attribute describing by how many step the variable will be changed in sensitivity analysis (required)
