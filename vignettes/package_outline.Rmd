...
vignette: >
  %\VignetteIndexEntry{godley — package outline}
...
There are a few steps to follow in order to create and simulate a baseline scenario.  


### 1. Model creation
Use the `create_model()` function to create an SFC model (SFC object):
...
*Note:* Users can bypass manually adding variables and equations (steps 2 and 3 below) by uploading model attributes from pre-defined templates included in the package. Available templates are: `SIM`, `SIMEX`, `PC`, `LP`, `REG`, `OPEN`, `BMW`, `BMWK`, `DIS`, and `DISINF`.


### 2. Adding variables
Use the `add_variable()` function to add variables to the model, creating new entries in the SFC tibble:
...
### 3. Adding equations
To add equations to the SFC model, also as new entries in the SFC tibble, use the `add_equation()` function:
...
To change the initial values of parameters, use the `change_init()` function:
...
Use the `simulate_scenario()` function to carry out the simulation for the specified scenarios:
...
## Model — Plots
To visualize model simulation, use the `plot_simulation()` function as shown below:
...
## Model — Shock
To create and simulate a shock scenario, follow these steps:

### 1. Creating shock object
Use the `create_shock()` function to create an SFC shock tibble object:
...
Use the `add_shock()` function to add shock equations to the SFC shock object:
...
Use the `add_scenario()` function to integrate the shock scenario into the SFC model:
...
Use the `simulate_scenario()` function to simulate the shock scenario:
...
## Model — sensitivity
Sensitivity analysis allows the creation of multiple scenarios in a separate SFC model by changing the initial value of a selected variable. To create and simulate sensitivity, use the `create_sensitivity()` and `simulate_scenario()` functions as shown below:
