...
model_sim <- add_variable(model = model_sim, name = "H_s", 
...
model_sim <- add_variable(model = model_sim, name = "alpha1", init = 0.6, 
                          desc = "Propensity to consume out of income")
model_sim <- add_variable(model = model_sim, name = "alpha2", init = 0.4, 
...
model_sim <- add_equation(model = model_sim, equation = "H_s = H_h", desc = "Money equilibrium", 
                          hidden = TRUE)
...
model_sim <- simulate_scenario(model = model_sim, scenario = "baseline", max_iter = 350, periods = 100, 
                               hidden_tol = 0.1, tol = 1e-08, method = "Gauss")

## ----fig, fig.width = 6, fig.high = 4, fig.asp = 1----------------------------
#plot results
plot_simulation(model = model_sim, scenario = 'baseline', from = 1, to = 100, 
...
## -----------------------------------------------------------------------------
#add and calculate shock scenario for increased government expenditures
model_sim <- add_scenario(model = model_sim, name = 'expansion', origin = 'baseline', 
                          origin_period = 1, shock = sim_shock)
model_sim <- simulate_scenario(model = model_sim, max_iter = 350, periods = 100, hidden_tol = 0.1, 
...
model_sen <- simulate_scenario(model = model_sen, max_iter = 350, periods = 100, hidden_tol = 0.1, 
                               tol = 1e-08, method = "Gauss")
