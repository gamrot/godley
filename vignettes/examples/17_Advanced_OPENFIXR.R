...
# An alternative fixed exchange rate closure: the deficit country lets its interest rate become endogenous
...
model_open <- model_open |>
  ...
  # Demand for US bills in UK - 12.68F - excluded
  ...
  add_equation("b_ukus_d = b_ukus_s*xr_us") |>
  # 12.90R : Supply of UK bills to US
  add_equation("b_ukus_s =  b_us_s - b_usus_s - b_cb_usus_d - b_cb_ukus_s") |>
  # 12.68R: Endogenous interest rate
  add_equation("r_uk = (lambda20 + lambda22*(r_us + dxre_us) - b_ukus_d/v_uk)/lambda21")

# Simulate the model
model_open <- simulate_scenario(model_open, scenario = "baseline", max_iter = 350,
                                periods = 100, tol = 1e-15, method = "Broyden")

# Scenario 1: Increase in the UK propensity to import
# Introduce a sustained increase in the import propensity parameter
...
# Simulate the adjustment path
model_open <- simulate_scenario(model_open, scenario = "mu0_shock", periods = 45,
                                  max_iter = 350, tol = 1e-10, method = "Broyden", rhtol = TRUE)
...
do_plotly(m = model_open, scenario = "mu0_shock",
        ...
        t0 = 1, end = 45, title = "Scenario: mu0_shock")
...
# and on the UK debt to GDP ratio
m1 <- model_open[["mu0_shock"]][["result"]] %>%
  dplyr::mutate(
    by_uk = b_uk_s / y_uk
    ) %>%
  dplyr::filter(time >= 1 & time <= 45)
...
plotly::plot_ly(m1, x = ~time) %>%
  plotly::add_lines(
    ...
    yaxis = "y2",
    line = list(width = 2, dash = "solid"),
    hovertemplate = "<b>r_uk</b><br>time=%{x}<br>value=%{y}<extra></extra>"
  ) %>%
  plotly::add_lines(
    y = ~by_uk,
    name = "by_uk",
    line = list(width = 2, dash = "dash"),
    hovertemplate = "<b>by_uk</b><br>time=%{x}<br>value=%{y}<extra></extra>"
  ) %>%
  plotly::layout(
  ...
  plotly_style(title = "Scenario: by_uk and r_uk shocks")
