...
# A fixed exchange rate closure, with endogenous foreign reserves
...
model_open <- model_open |>
  ...
  add_equation("yd_hs_uk = yd_uk + (xr_us - xr_us[-1])*b_ukus_s[-1]") |>
  
  # Wealth accumulation in UK - eq. 12.3
  add_equation("v_uk = v_uk[-1] + yd_uk - cons_uk") |>
  
  # Disposable income in US - eq. 12.4
  add_equation("yd_us = (y_us + r_us[-1]*b_usus_d[-1] + xr_uk*r_uk[-1]*b_usuk_s[-1])*(1 - theta_us) + (xr_uk - xr_uk[-1])*b_usuk_s[-1]") |>
  
  # Haig-Simons disposable income in US - eq. 12.5
  add_equation("yd_hs_us = yd_us + d(xr_uk)*b_usuk_s[-1]") |>
  
  # Wealth accumulation in US - eq. 12.6
  add_equation("v_us = v_us[-1] + yd_us - cons_us") |>
  
  # Taxes in UK - eq. 12.7
  add_equation("t_uk = theta_uk*(y_uk + r_uk[-1]*b_ukuk_d[-1] + xr_us*r_us[-1]*b_ukus_s[-1])") |>
  
  # Taxes in US - eq. 12.8
  add_equation("t_us = theta_us*(y_us + r_us[-1]*b_usus_d[-1] + xr_uk*r_uk[-1]*b_usuk_s[-1])") |>
  ...
  add_equation("f_cb_uk = r_uk[-1]*b_cb_ukuk_d[-1] + r_us[-1]*b_cb_ukus_s[-1]*xr_us") |>
  
  # Profits of Central Bank in US - eq. 12.12
  add_equation("f_cb_us = r_us[-1]*b_cb_usus_d[-1]") |>
  
  # Government budget constraint - UK - eq. 12.13
  add_equation("b_uk_s = b_uk_s[-1] + g_uk + r_uk[-1]*b_uk_s[-1] - t_uk - f_cb_uk") |>
  
  # Government budget constraint - US - eq. 12.14
  add_equation("b_us_s = b_us_s[-1] + g_us + r_us[-1]*b_us_s[-1] - t_us - f_cb_us") |>
  
  # Current account balance - UK - eq. 12.15
  add_equation("cab_uk = x_uk - im_uk + xr_us*r_us[-1]*b_ukus_s[-1] - r_uk[-1]*b_usuk_s[-1] + r_us[-1]*b_cb_ukus_s[-1]*xr_us") |>
  
  # Capital account balance in UK - eq. 12.16
  add_equation("kab_uk = kabp_uk - (xr_us*(b_cb_ukus_s - b_cb_ukus_s[-1]) + pg_uk*(or_uk - or_uk[-1]))") |>
  
  # Current account balance in US - eq. 12.17
  add_equation("cab_us = x_us - im_us + xr_uk*r_uk[-1]*b_usuk_s[-1] - r_us[-1]*b_ukus_s[-1] - r_us[-1]*b_cb_ukus_s[-1]") |>
  
  # Capital account balance in US - eq. 12.18
  add_equation("kab_us = kabp_us + (b_cb_ukus_s - b_cb_ukus_s[-1]) - pg_us*(or_us - or_us[-1])") |>
  
  # Capital account balance in UK, net of official transactions - eq. 12.19
  add_equation("kabp_uk = -(b_ukus_s - b_ukus_s[-1])*xr_us + (b_usuk_s - b_usuk_s[-1])") |>
  
  # Capital account balance in US, net of official transactions - eq. 12.20
  add_equation("kabp_us = -(b_usuk_s - b_usuk_s[-1])*xr_uk + (b_ukus_s - b_ukus_s[-1])") |>
  
  # TRADE
  # Import prices in UK - eq. 12.21
  add_equation("pm_uk = exp(nu0m + nu1m*log(py_us) + (1 - nu1m)*log(py_uk) - nu1m*log(xr_uk))") |>
  
  # Export prices in UK - eq. 12.22
  add_equation("px_uk = exp(nu0x + nu1x*log(py_us) + (1 - nu1x)*log(py_uk) - nu1x*log(xr_uk))") |>
  
  # Export prices in US - eq. 12.23
  add_equation("px_us = pm_uk*xr_uk") |>
  
  # Import prices in US - eq. 12.24
  add_equation("pm_us = px_uk*xr_uk") |>
  
  # Real exports from UK - eq. 12.25 - depends on current relative price
  add_equation("x_k_uk = exp(eps0 - eps1*log(pm_us/py_us) + eps2*log(y_k_us))") |>
  
  # Real imports of UK - eq. 12.26
  add_equation("im_k_uk = exp(mu0 - mu1*log(pm_uk[-1]/py_uk[-1]) + mu2*log(y_k_uk))") |>
  
  # Real exports from US - eq. 12.27
  add_equation("x_k_us = im_k_uk") |>
  
  # Real imports of US - eq. 12.28
  add_equation("im_k_us = x_k_uk") |>
  
  # Exports of UK - eq. 12.29
  add_equation("x_uk = x_k_uk*px_uk") |>
  
  # Exports of US - eq. 12.30
  add_equation("x_us = x_k_us*px_us") |>
  
  # Imports of UK - eq. 12.31
  add_equation("im_uk = im_k_uk*pm_uk") |>
  
  # Imports of US - eq. 12.32
  add_equation("im_us = im_k_us*pm_us") |>
  
  # INCOME AND EXPENDITURE
  # Real wealth in UK - eq. 12.33
  add_equation("v_k_uk = v_uk/pds_uk") |>
  
  # Real wealth in US - eq. 12.34
  add_equation("v_k_us = v_us/pds_us") |>
  
  # Real Haig-Simons disposable income in UK - eq. 12.35
  add_equation("ydhs_k_uk = yd_uk/pds_uk - v_k_uk[-1]*(pds_uk - pds_uk[-1])/pds_uk") |>
  
  # Real Haig-Simons disposable income in US - eq. 12.36
  add_equation("ydhs_k_us = yd_us/pds_us - v_k_us[-1]*(pds_us - pds_us[-1])/pds_us") |>
  
  # Real consumption in UK - eq. 12.37
  add_equation("c_k_uk = alpha1_uk*ydhse_k_uk + alpha2_uk*v_k_uk[-1]") |>
  
  # Real consumption in US - eq. 12.38
  add_equation("c_k_us = alpha1_us*ydhse_k_us + alpha2_us*v_k_us[-1]") |>
  
  # Expected real Haig-Simons disposable income in UK - eq. 12.39
  add_equation("ydhse_k_uk = (ydhs_k_uk + ydhs_k_uk[-1])/2") |>
  
  # Expected real Haig-Simons disposable income in US - eq. 12.40
  add_equation("ydhse_k_us = (ydhs_k_us + ydhs_k_us[-1])/2") |>
  
  # Real sales in UK - eq. 12.41
  add_equation("s_k_uk = c_k_uk + g_k_uk + x_k_uk") |>
  
  # Real sales in US - eq. 12.42
  add_equation("s_k_us = c_k_us + g_k_us + x_k_us") |>
  
  # Value of sales in UK - eq. 12.43
  add_equation("s_uk = s_k_uk*ps_uk") |>
  
  # Value of sales in US - eq. 12.44
  add_equation("s_us = s_k_us*ps_us") |>
  
  # Price of sales in UK - eq. 12.45
  add_equation("ps_uk = (1 + phi_uk)*(w_uk*n_uk + im_uk)/s_k_uk") |>
  
  # Price of sales in US - eq. 12.46
  add_equation("ps_us = (1 + phi_us)*(w_us*n_us + im_us)/s_k_us") |>
  
  # Price of domestic sales in UK - eq. 12.47
  add_equation("pds_uk = (s_uk - x_uk)/(s_k_uk - x_k_uk)") |>
  
  # Price of domestic sales in US - eq. 12.48
  add_equation("pds_us = (s_us - x_us)/(s_k_us - x_k_us)") |>
  
  # Domestic sales in UK - eq. 12.49
  add_equation("ds_uk = s_uk - x_uk") |>
  
  # Domestic sales in US - eq. 12.50
  add_equation("ds_us = s_us - x_us") |>
  
  # Real domestic sales in UK - eq. 12.51
  add_equation("ds_k_uk = c_k_uk + g_k_uk") |>
  
  # Real domestic sales in US - eq. 12.52
  add_equation("ds_k_us = c_k_us + g_k_us") |>
  
  # Value of output in UK - eq. 12.53
  add_equation("y_uk = s_uk - im_uk") |>
  
  # Value of output in US - eq. 12.54
  add_equation("y_us = s_us - im_us") |>
  
  # Value of real output in UK - eq. 12.55
  add_equation("y_k_uk = s_k_uk - im_k_uk") |>
  
  # Value of real output in US - eq. 12.56
  add_equation("y_k_us = s_k_us - im_k_us") |>
  
  # Price of output in UK - eq. 12.57
  add_equation("py_uk = y_uk/y_k_uk") |>
  
  # Price of output in US - eq. 12.58
  add_equation("py_us = y_us/y_k_us") |>
  
  # Consumption in UK - eq. 12.59
  add_equation("cons_uk = c_k_uk*pds_uk") |>
  
  # Consumption in US - eq. 12.60
  add_equation("cons_us = c_k_us*pds_us") |>
  
  # Government expenditure in UK - eq. 12.61
  add_equation("g_uk = g_k_uk*pds_uk") |>
  
  # Government expenditure in US - eq. 12.62
  add_equation("g_us = g_k_us*pds_us") |>
  
  # Note: The tax definitions (book eqs. 12.63–12.64) already appear here as eqs. 12.7–12.8.

  # Employment in UK - eq. 12.65
  add_equation("n_uk = y_k_uk/pr_uk") |>
  
  # Employment in US - eq. 12.66
  add_equation("n_us = y_k_us/pr_us") |>
  
  # ASSET DEMANDS
  # Demand for UK bills in UK - eq. 12.67
  add_equation("b_ukuk_d = v_uk*(lambda10 + lambda11*r_uk - lambda12*(r_us + dxre_us))") |>
  
  # Demand for US bills in UK - 12.68F
  add_equation("b_ukus_d = v_uk*(lambda20 - lambda21*r_uk + lambda22*(r_us + dxre_us))") |>
  ...
  add_equation("h_uk_d = v_uk - b_ukuk_d - b_ukus_d") |>
  
  # Demand for US	bills in US - eq. 12.70
  add_equation("b_usus_d = v_us*(lambda40 + lambda41*r_us - lambda42*(r_uk + dxre_uk))") |>
  
  # Demand for UK bills in US - eq. 12.71
  add_equation("b_usuk_d = v_us*(lambda50 - lambda51*r_us + lambda52*(r_uk + dxre_uk))") |>
  
  # Demand for money in US - eq. 12.72
  add_equation("h_us_d = v_us - b_usus_d - b_usuk_d") |>
...
shock_open <- create_shock() |>
  add_shock(variable = "eps0", value = -2, start = 5, end = 100)
...
model_open <- simulate_scenario(model_open, scenario = "eps0_shock", periods = 45,
...
# of financial assets, current account balance, trade balance and government budget balance
do_plotly(m = model_open, scenario = "eps0_shock",
        variables = c("cab_uk", "nafa_uk", "gab_uk", "tab_uk"),
        t0 = 1, end = 45, title = "Scenario: eps0_shock")

# Figure 12.1B Effect of an increase in the US propensity to import, within a fixed 
...
# to GDP ratio and on the UK debt to income ratio, within a fixed exchange rate regime
# with endogenous foreign reserves
do_plotly(m = model_open, scenario = "eps0_shock",
        variables = c("by_uk", "by_us"),
        t0 = 1, end = 50, title = "Scenario: eps0_shock")

# Scenario 2: A one-step devaluation after an increase in the UK propensity to import
# Define an increase in import propensity combined with a one-step devaluation
shock_open <- create_shock() |>
  add_shock(variable = "mu0", value = -2, start = 5, end = 100) |>
  add_shock(variable = "xr_uk", value = 0.84, start = 10, end = 100)

# Introduce the scenario and simulate it
model_open <- model_open |>
...
model_open <- simulate_scenario(model_open, scenario = "mu0-xr_uk_shock", periods = 100,
                                  ...
                                  hidden_tol = 0.1, rhtol = TRUE)
# Plot the results
# Figure 12.4A Effect on the UK current account balance and trade balance 
# of a successful one-step devaluation of the sterling pound, following an increase in the UK
# propensity to import, within a fixed exchange rate regime with endogenous foreign reserves
do_plotly(m = model_open, scenario = "mu0-xr_uk_shock",
        variables = c("tab_uk", "cab_uk"),
        t0 = 1, end = 50, title = "Scenario: mu0 and xr_uk shocks")

# Figure 12.4B Effect of a successful one-step devaluation of the sterling pound on UK
...
plotly::plot_ly(m1, x = ~time) %>%
  plotly::add_lines(
    ...
    name = "y_k_uk",
    line = list(width = 2, dash = "solid"),
    hovertemplate = "<b>y_k_uk</b><br>time=%{x}<br>value=%{y}<extra></extra>"
  ) %>%
  plotly::add_lines(
    y = ~xr_uk,
    name = "xr_uk",
    ...
    hovertemplate = "<b>xr_uk</b><br>time=%{x}<br>value=%{y}<extra></extra>"
  ) %>%
  plotly::layout(
    yaxis  = list(title = "real output"),
    yaxis2 = list(
      title = "exchange rate",
      overlaying = "y",
      side = "right"
  ...
  plotly_style(title = "Scenario: mu0 and xr_uk shocks")
