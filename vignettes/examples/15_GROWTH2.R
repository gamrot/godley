...
model_growth <- model_growth |>
  ...
  # Initialize: Rrbt, Rrb (epsrb is already set to 0.9)
...
model_growth <- model_growth |>
  ...
  add_equation("YP = WB + FDf + FDb + Rm[-1]*Mh[-1] + Rb[-1]*Bhd[-1] + BLs[-1]", desc = "11.45 : Personal income") |>
  ...
  add_equation("Pe = Vfma[-1]*(lambda40 - lambda42*Rb[-1] - lambda41*Rm[-1] + lambda44*Rk[-1] - lambda43*Rbl[-1] - lambda45*(YDr/V))/Ekd", desc = "11.66 : Demand for equities - normalized to get the price of equities") |>
...
# Add auxiliary variables required by the sfcr structure but not explicitly initialised
model_growth <- model_growth |>
  add_variable("z3") |>
  add_variable("z4") |>
  add_variable("z5") |>
...
# Scenario: Permanent increase in government expenditures
# Define the fiscal shock path and add it as an alternative scenario
...
# Simulate the counterfactual fiscal expansion
model_growth <- simulate_scenario(model_growth, scenario = "GRg_shock", periods = 100, 
                                  max_iter = 350, tol = 1e-10, method = "Broyden",
                                  hidden_tol = 0.1, rhtol = TRUE)
# Plot the results
# Figure 11.6A Evolution of the growth rate of real output, with the growth rate of real
...
plot_simulation(model = model_growth, scenario = c("GRg_shock"),
...
# manner
do_cplotly(m = model_growth, scenario = "GRg_shock",
         from = 1, to = 25, variables = c("Gk", "ER"))
...
plot_simulation(model = model_growth, scenario = c("GRg_shock"),
                from = 1, to = 25, expressions = c("Rl", "Rb", "Rm"))
