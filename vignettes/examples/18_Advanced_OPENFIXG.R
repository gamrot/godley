...
# Another alternative fixed exchange rate closure: one country sets its government expenditures endogenously
...
model_open <- model_open |>
  ...
  add_equation("g_uk = b_uk_s - b_uk_s[-1] -(r_uk[-1]*b_uk_s[-1] - t_uk - f_cb_uk)") |>
  # 12.61G: Real government expenditures
  add_equation("g_k_uk = g_uk / pds_uk") |>
  # 12.82AG: Supply of UK Bills
  add_equation("b_uk_s = b_usuk_s + b_ukuk_s + b_cb_ukuk_s")
...
# Simulate the model under this scenario
model_open <- simulate_scenario(model_open, scenario = "mu0_shock", periods = 100,
                                  max_iter = 350, tol = 1e-15, method = "Broyden",
                                  hidden_tol = 0.1)
# Plot the results
# Figure 12.3A Effect of an increase in the UK propensity to import, within a fixed
# exchange rate regime with endogenous UK government expenditures, on the US and UK real GDP
do_plotly(m = model_open, scenario = "mu0_shock",
        variables = c("y_k_us", "y_k_uk"),
        t0 = 1, end = 50, title = "Scenario: mu0_shock")

# Figure 12.3B Effect of an increase in the UK propensity to import, within a fixed
# exchange rate regime with endogenous UK government expenditures, on various UK
# variables: the capital account balance, the current account balance and the trade balance
do_plotly(m = model_open, scenario = "mu0_shock",
...
# exchange rate regime with endogenous UK government expenditures, on the UK 
# and US debt to GDP ratios
do_plotly(m = model_open, scenario = "mu0_shock",
