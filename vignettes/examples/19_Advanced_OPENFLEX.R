...
# A flexible exchange rate closure
...
model_open <- model_open |>
  ...
  # Add variables used in the model but not explicitly declared
...
model_open <- model_open |>
  ...
  add_equation("xr_uk = b_ukus_s/b_ukus_d", desc = "") |>
  # 12.90G : Supply of UK bills to us
  add_equation("b_ukus_s = b_us_s - b_usus_s - b_cb_usus_d - b_cb_ukus_s", desc = "") |>
...
do_plotly(m = model_open, scenario = "eps0_shock",
          ...
          title = "Scenario: eps0_shock")

# Figure 12.5B Effect on the sterling exchange rate of a decrease in the UK propensity
# to export, within a flexible exchange rate regime
do_plotly(m = model_open, scenario = "eps0_shock",
          variables = c("xr_uk"), title = "Scenario: eps0_shock")

# Figure 12.5C Effect of a decrease in the UK propensity to export, within a flexible
# exchange rate regime, on various UK price indices: export prices, import prices 
# and domestic sales prices
do_plotly(m = model_open, scenario = "eps0_shock",
          variables = c("pm_uk", "px_uk", "pds_uk"),
          title = "Scenario: eps0_shock")

# Figure 12.5D Effect of a decrease in the UK propensity to export, within a flexible
# exchange rate regime, on the UK and US real GDP
do_plotly(m = model_open, scenario = "eps0_shock",
        variables = c("y_k_uk", "y_k_us"), title = "Scenario: eps0_shock")

# Note: From this point onward, we follow the same steps as in the first simulation, so comments are kept to a minimum.

# Scenario 2: A step increase in US government expenditures
shock_open <- create_shock() |>
  add_shock(variable = "g_k_us", value = 18, start = 5, end = 100, desc = "")

model_open <- model_open |>
  add_scenario(name = "g_k_us_shock", origin = "baseline", shock = shock_open)

model_open <- simulate_scenario(model_open, scenario = "g_k_us_shock", periods = 100,
                                max_iter = 350, tol = 1e-10, method = "Broyden",
                                hidden_tol = 0.1, rhtol = TRUE)
# Results
# Figure 12.6A Effect of a step increase in real US government expenditures, 
# within a flexible exchange rate regime, on the US and UK real GDP
do_plotly(m = model_open, scenario = "g_k_us_shock",
        variables = c("y_k_us", "y_k_uk"), title = "Scenario: g_k_us_shock")

# Figure 12.6B Effect of a step increase in real US government expenditures, 
# within a flexible exchange rate regime, on the main US balances: net accumulation 
# of financial assets, government budget deficit, current account balance
do_plotly(m = model_open, scenario = "g_k_us_shock",
        variables = c("nafa_us", "psbr_us", "cab_us"),
        title = "Scenario: g_k_us_shock")
...
do_plotly(m = model_open, scenario = "g_k_us_shock",
        variables = c("bukus_p", "bukus_d"), title = "Scenario: g_k_us_shock")

# Figure 12.6D Effect on the dollar exchange rate of a step increase in real US
# government expenditures, within a flexible exchange rate regime
do_plotly(m = model_open, scenario = "g_k_us_shock",
          variables = c("xr_us"), title = "Scenario: g_k_us_shock")
...
model_open <- simulate_scenario(model_open, scenario = "lambda20-40_shock", periods = 100,
                                ...
                                hidden_tol = 0.1, rhtol = TRUE)
# Results
# Figure 12.7A Effect on the dollar exchange rate of an increase in the desire
# to hold US Treasury bills, within a flexible exchange rate regime
do_plotly(m = model_open, scenario = "lambda20-40_shock",
        variables = c("xr_us"), title = "Scenario: lambda20 and lambda40 shocks")

# Figure 12.7B Effect of an increase in the desire to hold US Treasury bills, within a
# flexible exchange rate regime, on the share of the wealth of UK residents held in the
# form of US Treasury bills, when denominated in dollars and then in sterling
do_plotly(m = model_open, scenario = "lambda20-40_shock",
        variables = c("bukus_p", "bukus_d"),
        title = "Scenario: lambda20 and lambda40 shocks")

# Figure 12.7C Effect on UK and US real GDP of an increase in the desire to hold US
...
do_plotly(m = model_open, scenario = "lambda20-40_shock",
          variables = c("psbr_us", "cab_us", "nafa_us", "tab_us"),
          title = "Scenario: lambda20 and lambda40 shocks")
